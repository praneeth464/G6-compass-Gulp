  ...
  Modal.prototype = {
    ...
    , show: function () {
        ...
        if(this.$element.hasClass('modal-stack')) {
          if(this.$element.data('yOffset') === 'adjust') {
            // pageYOffset works for most, but < ie9 needs $.scrollTop() applied to 'html' -- chrome uses 'body'
            this.$element.css('top', window.pageYOffset||$('html').scrollTop());
          } else if(this.$element.closest('.page-content').length) {
            this.$element.css('top', this.$element.closest('.page-content').offset().top);
          }
    ...
    , hideModal: function () {
        ...
        this.backdrop(function () {
          ...
          $('body').trigger('modal-hidden', that.$element)
